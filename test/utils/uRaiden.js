...
async function createChannel(uRaiden, ttc, spender, recipient, amount) {
  ...
  assert.equal(channelCreatedEvent.args._receiver_address, recipient, 'ChannelCreated receiver')
...
async function testDepositTopUp(uRaiden, ttc, ttm, owner, channel, amount) {
  ...
  const channelToppedUpEvents = result.logs
	.filter( l => l.event == 'ChannelToppedUp')
  ...
  assert.equal(tuEvent._added_deposit.toString(), amount.toString(), 'deposit wrong')
  ...
  assert.equal(channelInfo.deposit, new BigNumber(channel.channelInfo.deposit).add(amount).toString())
  return { ...channel, channelInfo}
}
...
async function testCreateChannel(uRaiden, ttc, spender, recipient, amount) {
  try {
    ...
    assert.equal(preBalances.recipient.toString(), postBalances.recipient.toString(), 'recipient balance remains untouched')
