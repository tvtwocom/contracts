...
const testWillThrow = async (fn, args) => {
  try {
    if (args instanceof Array) await fn(...args)
    else await fn
...
function hasEvent(receipt, eventName) {
  ...
  assert(
    events.length > 0,
    `${JSON.stringify(receipt, null, 2)} does not contain ${eventName}`
  )
