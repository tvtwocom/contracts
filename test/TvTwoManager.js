...
const {
  ...
  testSetPaywall,
  testSetTTCoin
} = require('./utils/manage')
const { testCreateVideo, testSetMinAllowance } = require('./utils/ttm')

const { testBuyTokens } = require('./utils/ttc')
...
describe('when creating videos', () => {
  contract('TvTwoManager', accounts => {
    ...
    it('should NOT create ad if NO channel given', async () => {
      ...
      await testWillThrow(
        testCreateVideo(ttm, adHash, isAd, openingBlock, advertiser)
      )
    })
    ...
    it('should not create an ad if called by a contract', async () => {
      ...
      await testWillThrow(
        proxy.addVideo(adHash, isAd, channelInfo.openingBlock)
      )
      await ttm.addVideo(adHash, isAd, channelInfo.openingBlock, {
        from: advertiser
      })
    })
    ...
    it('should create an ad if there had been withdrawls on the channel', async () => {
      ...
      await testWithdrawl(uRaiden, ttc, channelInfo)
    ...
    it('should NOT create an ad if channel is closed but not yet settled', async () => {
      ...
      const isAd = true
      await testBuyTokens(ttc, advertiser, buyAmount)
      const channel = await testCreateChannel(
        uRaiden,
        ttc,
        advertiser,
        paywall,
      ...
      await uRaiden.uncooperativeClose(
        channel.recipient,
        channel.openingBlock,
        minTokenAmount,
        { from: channel.spender }
      )
    ...
    it('should NOT create the same video again with same hash even if other attributes different', async () => {
      ...
      const otherChannel = await testCreateChannel(
        uRaiden,
        ttc,
        contentCreator,
        paywall,
...
describe('when reaching checkpoints', async () => {
  contract('TvTwoManager/TvTwoCoin', accounts => {
    ...
    it('should emit event with the right properties', async () => {
      const result = await ttm.channelCheckpoint(
        spender,
        recipient,
        balance,
        opening_block_number,
        logsHash,
        { from: owner }
      )
      ...
      assert.equal(
        event.args._opening_block_number.toString(),
        opening_block_number.toString()
      )
